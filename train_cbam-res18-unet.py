...
from unet.CBAMRes18UNet import CBAMResnet18_Unet
...
# 可视化配置
using_wandb = True
wandb_project = 'unet-2k-c'
wandb_name = 'cbam-res18-unet-pre-untitled'
    ...
    for epoch in range(epoch_start, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    ...
                    loss = cml_loss(model, masks_pred, true_masks, model.n_classes, beta=beta)
        ...
        if save_checkpoint:
            ...
            for i,batch in enumerate(tqdm(test_loader, total=len(test_loader), desc='Test round', unit='batch', leave=True)):
                ...
                test_loss += cml_loss(model, mask_pred, true_mask, model.n_classes, beta=beta)
                if model.n_classes > 1:
                ...
                Image.fromarray((image[0,0].cpu().numpy()*255).astype('u1')).save(Path(save_path).with_name(f'test{i:04d}.png'))
                plot_and_save_pred(None, mask_pred.squeeze(0).cpu().numpy(), true_mask.squeeze(0).cpu().numpy(), save_path, viz=False, methods=('diff_rgb'))
...
if __name__ == '__main__':
    ...
    model = CBAMResnet18_Unet(n_channels=args.channels, n_classes=args.classes, pretrained=True)
